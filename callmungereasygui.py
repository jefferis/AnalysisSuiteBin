...
print scriptPath
textbox(msg='scriptPath = %s' % (scriptPath), title=' ', text='', codebox=0)
...
image=diropenbox("Choose the source images directory", 
...
refBrain = fileopenbox("Choose the reference brain: this can be a PIC or a nrrd file","Select Reference Brain")
print refBrain
refBrain=relpath(refBrain,rootDir)
print refBrain

# 3) Select additional flags
mungeropts=enterbox(msg='Please enter the options to be passed to munger', default='-a -w -r 01 -v', strip=True)

# multchoicebox(msg='What actions do you want', title=' ', choices=('affine','warp','reformat'))
# 4) Action
action=buttonbox(msg='Choose an action', choices=('Test', 'Run', 'Write script','Cancel'))
