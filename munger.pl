...
sub runLandmarksAffine {
	...
	myexec ( "mkdir", "-p", $outlist ) unless $opt{t};
	makelock("$outlist/registration.lock");
	# swapped order of sample and ref because the resultant affine transforms
	# seemed to be the inverse of those generated by Torsten's
	# make_initial_affine and registration tools
	my @cmd=( $landmarksAffCommand, $args, $sampleLandmarks, $refLandmarks, $outlist );
	my $cmd_string = join( ' ', @cmd );
	if( $opt{v}){
